...
For final submission verification, use `run_inference.run_inference()` with one of the named submission variants. These calls rebuild the selected result JSONL and write the final `id,response` CSV through the single entry point.

Primary confirmed private-LB variant:
...
run_inference(
    ...
    variant="learned_selector_precision_exact_v1",
)
```

Second-slot inference-consistency variant:
...
run_inference(
    data_path="private.jsonl",
    variant="mcq_shuffle_all_conservative_free_v1",
)
```
...
The default call without `variant` still regenerates the baseline `results/private_4b/submission.csv`; use the variant calls above for the two final CSVs recommended for submission.
...
The required entry point is [run_inference.py](run_inference.py). It exposes the final submission variants:

```python
from run_inference import run_inference

run_inference(data_path="private.jsonl", variant="learned_selector_precision_exact_v1")
run_inference(data_path="private.jsonl", variant="mcq_shuffle_all_conservative_free_v1")
```

The baseline generation path and optional adapter/hybrid mode are also contained in the same function:

```python
run_inference(data_path="private.jsonl", output_dir="results/private_4b")
```

Optional adapter/hybrid mode:
...
The baseline path performs the full generation pipeline:
...
8. Merges GPU shards.
9. Writes an `id,response` submission CSV.

The final submission variants then run the selected post-processing workflow inside `run_inference()`: learned candidate selection, exact-free source routing, option-shuffle MCQ overlay, combination with the included conservative free-form overlay, validation, and CSV export. The candidate source JSONLs used by these variants are included under `results/` and were generated by the Qwen3-4B inference scripts in this repository.

## CLI Usage

Place `private.jsonl` at the repository root, then run the two final submission variants:

```bash
python3 run_inference.py --data private.jsonl --variant learned_selector_precision_exact_v1
python3 run_inference.py --data private.jsonl --variant mcq_shuffle_all_conservative_free_v1
```

For the baseline generation-only path:
...
The following files record private leaderboard experiments and ablations. The two recommended final CSVs are now reproducible through `run_inference(..., variant=...)`; the remaining files are retained as analysis artifacts and backups.
